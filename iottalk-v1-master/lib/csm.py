...
@app.route('/csm_snapshot', methods=['GET'])
def csm_snapshot():
    snapshot = open('csm_state_snapshot', 'w')
    snapshot.write(str(datetime.now())+'\n')
...
snapshot_thx = threading.Thread(target=snapshot, args=(120, snapshot_period)) #Wait 60 seconds for ESM state restoring
